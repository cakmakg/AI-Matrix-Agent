...
│       ├── agents/                    # 13 AI agent node functions (LangGraph nodes)
│       ├── workflows/
...
│       ├── controllers/               # 17 Express route handlers
│       ├── routes/                    # 17 route modules (all mounted under /api)
│       ├── models/                    # 14 Mongoose schemas
│       ├── services/                  # 9 utility services
│       ├── middleware/                # tenant.js, rateLimiter.js, webhook.js
...
└── frontend/        # Next.js 14 App Router (TypeScript)
    └── src/
        ...
        ├── components/                # 32 TSX components organized by feature
        └── store/agent-store.ts       # Zustand store (single source of truth, ~1200 lines)
```
...
| **Client.js** | `name, slug, apiKey, plan, product, email, passwordHash` | Multi-tenant client registry. `plan`: free/pro/enterprise/holding. `product`: 12 SaaS product types. |
...
| **TenantConfig.js** | `clientId, configObject` | Multi-tenant configuration overrides (skills, feature flags, persona). |
...
| **costTracker.js** | Tracks LLM token usage → USD cost. Logs to `Transaction` collection. |
...
| `/auth` | authController | `POST /register`, `POST /login` |
...
// Navigation
activeView: "control" | "cfo" | "knowledge" | "settings" | "skills" | "social" | "security" | "auditor" | "supply";
